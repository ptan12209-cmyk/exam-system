...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName('bai')
        .setDescription('Thống kê tổng quan đề thi')
        .addStringOption(option =>
          option.setName('ten_de')
            .setDescription('Từ khóa tên đề thi')
            .setRequired(false)))
    .addSubcommand(subcommand =>
      subcommand
        .setName('lop')
        .setDescription('Thống kê lớp học (học sinh và thí sinh tự do)')
        .addStringOption(option =>
          option.setName('lop')
            .setDescription('Tên lớp học hoặc TSTD')
            .setRequired(false)))
    .addSubcommand(subcommand =>
      subcommand
        .setName('diem')
        .setDescription('Danh sách điểm chi tiết của học sinh theo đề thi')
        .addStringOption(option =>
          option.setName('ten_de')
            .setDescription('Từ khóa tên đề thi')
            .setRequired(true))),
  ...
  if (interaction.commandName === 'thongke') {
    ...
    if (subcommand === 'lop') {
      ...
      if (err || !students || students.length === 0) {
        return await interaction.followUp({ content: `📭 Không tìm thấy học sinh nào ${lopOption ? `thuộc lớp **${lopOption}**` : ''}.`, ephemeral: true });
      }
      ...
      if (hsLop.length > 0) {
        ...
        hsLop.forEach((s, idx) => {
          desc += `${idx + 1}. **${s.full_name}** (${s.class || 'Chưa rõ'})\n`;
        });
      }
      if (tstd.length > 0) {
        if (desc) desc += '\n';
        ...
        tstd.forEach((s, idx) => {
          desc += `${idx + 1}. **${s.full_name}**\n`;
        });
      }
      
      if (desc.length > 4000) {
        desc = desc.substring(0, 3990) + '\n...(Còn tiếp)';
      }
    ...
    if (subcommand === 'diem') {
      ...
      if (exErr || !exams || exams.length === 0) {
        return await interaction.followUp({ content: `❌ Không tìm thấy đề thi nào khớp với từ khóa **${ten_de}**`, ephemeral: true });
      }
      ...
      if (subError || !submissions || submissions.length === 0) {
        return await interaction.followUp({ content: `📭 Chưa ghi nhận bài nộp nào cho đề thi **${targetExam.title}**.`, ephemeral: true });
      }
