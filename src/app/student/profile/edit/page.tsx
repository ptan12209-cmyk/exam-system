...
export default function StudentProfileEditPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { error: updateError } = await supabase.from("profiles").update({ full_name: formData.full_name, nickname: formData.nickname || null, class: formData.class || null, bio: formData.bio || null, phone: formData.phone || null }).eq("id", user.id)
            if (updateError) { if (updateError.code === "23505") throw new Error("Biệt danh đã được sử dụng"); throw updateError }
            setSuccess(true); setTimeout(() => router.push("/student/profile"), 1500)
        ...
        <div className="min-h-screen bg-background py-8">
            <div className="max-w-2xl mx-auto px-4">
                <div className="flex items-center gap-4 mb-6">
                    <Link href="/student/profile"><Button variant="ghost" size="icon" className="text-muted-foreground hover:text-foreground"><ArrowLeft className="w-5 h-5" /></Button></Link>
                ...
                <form onSubmit={handleSubmit} className="glass-card rounded-2xl p-6 space-y-6">
                    ...
                    <div><Label htmlFor="nickname" className="text-foreground">Biệt danh</Label>
                        <Input id="nickname" value={formData.nickname} onChange={(e) => setFormData(prev => ({ ...prev, nickname: e.target.value }))} placeholder="vd: hoc_sinh_gioi" maxLength={20} className="mt-2 bg-card border-border text-foreground" />
                        <p className="text-xs text-muted-foreground mt-1">3-20 ký tự, chỉ chữ cái, số và dấu gạch dưới</p>
                    </div>
                    <div><Label htmlFor="class" className="text-foreground">Lớp</Label>
                        <Input id="class" value={formData.class} onChange={(e) => setFormData(prev => ({ ...prev, class: e.target.value }))} placeholder="vd: 12A1" className="mt-2 bg-card border-border text-foreground" />
                    </div>
                    <div><Label htmlFor="bio" className="text-foreground">Giới thiệu bản thân</Label>
                        <Textarea id="bio" value={formData.bio} onChange={(e) => setFormData(prev => ({ ...prev, bio: e.target.value }))} placeholder="Viết vài dòng về bản thân..." maxLength={200} rows={4} className="mt-2 bg-card border-border text-foreground" />
                    ...
                    <div className="flex gap-3 pt-4">
                        <Link href="/student/profile" className="flex-1"><Button type="button" variant="outline" className="w-full border-border text-muted-foreground" disabled={saving}>Hủy</Button></Link>
