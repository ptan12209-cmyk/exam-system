...
export default function TakeIeltsTestPage(props: { params: Promise<{ id: string }> }) {
  ...
  const handleAutoSubmit = async () => {
    ...
    if (result) {
      window.location.href = `/student/ielts/${testId}/result?submissionId=${result.submission_id}`
    }
  }
    ...
    <div className="container mx-auto p-4 sm:p-6 space-y-6 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
        ...
        <div className="flex items-center gap-3 self-end sm:self-auto shrink-0">
          ...
          <button
            ...
            disabled={submitting}
            className={`px-5 py-2 rounded-xl font-bold text-xs shadow-md transition-all active:scale-95 disabled:opacity-50 ${
              test.skill === 'reading' 
      ...
      {confirmSubmit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="glass-card w-full max-w-sm rounded-2xl border border-white/10 p-6 space-y-4 shadow-2xl">
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                ...
                className="flex-1 py-2 rounded-xl border border-white/10 text-xs font-semibold text-muted-foreground hover:bg-white/5 transition-all"
              >
                Hủy bỏ
              </button>
              <button
                ...
                disabled={submitting}
                className={`flex-1 py-2 rounded-xl font-bold text-xs text-white shadow-md transition-all ${
                  test.skill === 'reading' 
