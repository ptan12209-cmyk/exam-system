  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
      ...
      <div className="lg:col-span-7 space-y-6">
        <div className="glass-card p-6 rounded-2xl border border-white/10 bg-neutral-900/60 space-y-4">
          <div className="flex items-center justify-between border-b border-white/5 pb-3">
            <h3 className="text-sm font-bold text-foreground flex items-center gap-1.5">
              <Award className="h-4.5 w-4.5 text-orange-400" /> Điểm số thành phần
            </h3>
            <span className="text-[10px] font-bold text-orange-400 bg-orange-400/10 px-2 py-0.5 rounded border border-orange-500/20">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {criteria.map((c, idx) => (
              <div 
                key={idx}
                className={`glass-card p-4 rounded-xl border flex flex-col justify-between ${c.color}`}
              >
                <div>
                  <div className="flex items-center justify-between border-b border-white/5 pb-2 mb-2">
                    <span className="font-bold text-xs">{c.title}</span>
                    <span className="px-2 py-0.5 rounded bg-black/25 text-xs font-black">
          ...
          {scoreDetail.feedback_overall && (
            <div className="glass-card p-5 rounded-2xl border border-cyan-500/20 bg-cyan-500/5 space-y-3">
              <div className="flex items-center gap-1.5 text-cyan-400 border-b border-white/5 pb-2.5">
      ...
      <div className="lg:col-span-5 flex flex-col">
        <div className="glass-card border border-white/10 rounded-2xl p-5 bg-neutral-900/40 h-full flex flex-col justify-between min-h-[500px]">
          <div className="space-y-4 flex-1 flex flex-col">
            ...
            {/* Display Content */}
            <div className="flex-1 overflow-y-auto max-h-[420px] rounded-xl bg-black/35 border border-white/5 p-4 text-sm leading-relaxed whitespace-pre-wrap select-none text-foreground">
              {activeTab === 'essay' ? (
                essayContent
              ) : (
                <div className="space-y-3">
                  <div className="flex items-center gap-1 text-[10px] text-emerald-400 bg-emerald-500/5 px-2 py-1 rounded w-fit border border-emerald-500/10 font-bold uppercase">
          ...
          <div className="text-right text-xs text-muted-foreground pt-4 border-t border-white/5">
            {activeTab === 'essay' ? (
