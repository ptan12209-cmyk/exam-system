...
export function TestCard({ test }: TestCardProps) {
  ...
  return (
    <div className="p-5 rounded-[2rem] border border-[hsl(var(--border))]/60 bg-[hsl(var(--card))] hover:border-[hsl(var(--border))] hover:shadow-md transition-all duration-300 flex flex-col justify-between group relative">
      <div className="space-y-3">
        {/* Title & Badge */}
        <div className="flex items-start justify-between gap-3">
          <h4 className="font-bold text-sm sm:text-base text-foreground leading-tight group-hover:text-cyan-400 transition-colors line-clamp-2">
            {test.title}
          </h4>
        </div>
      ...
      <div className="pt-5 mt-auto">
        <Link href={`/student/ielts/${test.id}/take`}>
          <button className={`w-full py-2.5 px-4 rounded-full text-xs font-bold transition-all duration-300 flex items-center justify-center gap-1.5 shadow-md active:scale-98 ${theme.btn}`}>
