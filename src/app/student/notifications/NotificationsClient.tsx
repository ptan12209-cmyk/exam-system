...
import { Bell, Check, ExternalLink, FileText, User, BarChart3 } from "lucide-react"
...
export function NotificationsClient({ fullName, studentStats, initialNotifications }: NotificationsClientProps) {
  ...
  const quickLinks = [
    ...
    { href: "/student/analytics", label: "Thống kê", icon: BarChart3 },
  ...
  // Optimistic update — the server write happens in the background.
  ...
  const markAllAsRead = async () => {
    ...
    await supabase.from("notifications").update({ is_read: true }).eq("user_id", (await supabase.auth.getUser()).data.user?.id ?? "").eq("is_read", false)
  }
                ...
                >
                  <div className="flex gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      {notification.link ? (
                        <Link href={notification.link} onClick={() => !notification.is_read && void markAsRead(notification.id)} className="group block">
                        ...
                        <button onClick={() => !notification.is_read && void markAsRead(notification.id)} className="block w-full text-left">
