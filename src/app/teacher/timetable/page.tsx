      ...
      <main className="mx-auto max-w-7xl px-4 pb-24 pt-24 sm:px-6 lg:px-8 lg:py-10">
        ...
        <section className="mb-8 grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
          <div>
            <p className="mb-3 inline-flex items-center gap-2 rounded-full border border-[var(--os-muted)]/20 bg-[var(--os-card)] px-4 py-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--os-muted)]">
              <Calendar className="h-3.5 w-3.5 text-[var(--os-accent)]" /> Timetable Manager
            </p>
          ...
          <div className="bg-[var(--os-card)] border border-[var(--os-muted)]/20 rounded-xl p-5 shadow-sm flex flex-col justify-between">
            <div>
              <p className="text-xs font-mono uppercase text-[var(--os-muted)]">Tổng số ca dạy/học</p>
              <div className="mt-1 text-3xl font-bold text-[var(--os-fg)]">{entries.length} ca</div>
            </div>
          </div>
        </section>
        ...
        <div className="mb-6">
          <Button 
            ...
            className="rounded-xl bg-[var(--os-accent)] hover:bg-[var(--os-accent)]/90 text-[var(--os-accent-fg)] px-5 py-5 text-xs font-bold shadow-md gap-2"
          >
        ...
        {/* Timetable Grid View */}
        <section className="rounded-xl border border-[var(--os-muted)]/20 bg-[var(--os-card)] overflow-hidden shadow-md">
          <div className="overflow-x-auto">
            <div className="min-w-[900px]">
              
              {/* Grid Header Days */}
              <div className="grid grid-cols-7 border-b border-[var(--os-muted)]/20 bg-[var(--os-bg)]/30">
                {[1, 2, 3, 4, 5, 6, 0].map((dayIdx) => (
                  <div 
                    key={dayIdx} 
                    className={cn(
                      "border-r border-[var(--os-muted)]/10 px-3 py-4 text-center last:border-r-0",
                      dayIdx === currentDayOfWeek ? "bg-[var(--os-accent)]/10 text-[var(--os-accent)]" : "text-[var(--os-fg)]"
                    )}
                  >
                    <p className="text-xs font-bold tracking-wider">{DAYS[dayIdx]}</p>
                    <p className="text-[10px] text-[var(--os-muted)] font-mono mt-0.5">{entriesByDay[dayIdx].length} ca</p>
                  </div>
                ))}
              </div>
              ...
              <div className="grid min-h-[460px] grid-cols-7">
                {[1, 2, 3, 4, 5, 6, 0].map((dayIdx) => (
                  <div 
                    key={dayIdx} 
                    className={cn(
                      "space-y-3 border-r border-[var(--os-muted)]/10 p-3 last:border-r-0 bg-[var(--os-bg)]/5",
                      dayIdx === currentDayOfWeek ? "bg-[var(--os-accent)]/5" : ""
                    )}
                  >
                    ...
                    {entriesByDay[dayIdx].length === 0 && (
                      <div className="flex h-full min-h-[120px] items-center justify-center text-[var(--os-muted)]/15">
        ...
        {showForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-2xl border border-[var(--os-muted)]/20 bg-[var(--os-card)] p-6 space-y-4 text-[var(--os-fg)] shadow-2xl">
              <div className="flex items-center justify-between border-b border-[var(--os-muted)]/10 pb-3">
                <h2 className="text-base font-bold text-[var(--os-fg)]">{editingId ? "Cập nhật ca giảng dạy" : "Thêm ca dạy học mới"}</h2>
                <button onClick={resetForm} className="rounded-full p-1.5 hover:bg-[var(--os-muted)]/20 text-[var(--os-muted)]">
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              <div className="space-y-3.5">
                ...
                <div className="space-y-1">
                  <Label className="text-xs font-bold text-[var(--os-muted)] uppercase tracking-wider font-mono">Ngày trong tuần</Label>
                  <select 
                    value={formDay} 
                    onChange={(e) => setFormDay(Number(e.target.value))} 
                    className="w-full rounded-xl border border-[var(--os-muted)]/30 bg-[var(--os-bg)] px-3 py-2.5 text-sm text-[var(--os-fg)] outline-none focus:border-[var(--os-accent)]"
                  >
                    {DAYS.map((d, i) => (
                      <option key={i} value={i} className="bg-[var(--os-card)]">
                        {d}
                      </option>
                    ))}
                  </select>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label className="text-xs font-bold text-[var(--os-muted)] uppercase tracking-wider font-mono">Thời gian bắt đầu</Label>
                    <Input 
                      type="time" 
                      value={formStart} 
                      onChange={(e) => setFormStart(e.target.value)} 
                      className="rounded-xl border-[var(--os-muted)]/30 bg-[var(--os-bg)] text-[var(--os-fg)] focus:border-[var(--os-accent)]" 
                    />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs font-bold text-[var(--os-muted)] uppercase tracking-wider font-mono">Thời gian kết thúc</Label>
                    <Input 
                      type="time" 
                      value={formEnd} 
                ...
                {/* Subject name */}
                <div className="space-y-1">
                  <Label className="text-xs font-bold text-[var(--os-muted)] uppercase tracking-wider font-mono">Môn học/Tiết học *</Label>
                  <Input 
                    value={formSubject} 
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label className="text-xs font-bold text-[var(--os-muted)] uppercase tracking-wider font-mono">Lớp học</Label>
                    <Input 
                      value={formClass} 
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs font-bold text-[var(--os-muted)] uppercase tracking-wider font-mono">Phòng học</Label>
                    <Input 
                      value={formRoom} 
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-bold text-[var(--os-muted)] uppercase tracking-wider font-mono">Màu hiển thị</Label>
                    ...
                    {COLORS.map((c) => (
                      <button 
                        ...
                        className={cn(
                          ...
                          formColor === c ? "ring-2 ring-offset-2 ring-offset-[var(--os-card)] ring-[var(--os-accent)] scale-110" : "hover:scale-105"
                        )} 
                ...
                {/* Note */}
                <div className="space-y-1">
                  <Label className="text-xs font-bold text-[var(--os-muted)] uppercase tracking-wider font-mono">Ghi chú thêm</Label>
                  <Input 
                    value={formNote} 
              ...
              <div className="flex gap-3 pt-3 border-t border-[var(--os-muted)]/10">
                <Button 
                  onClick={handleSave} 
                  disabled={!formSubject.trim()} 
                  className="flex-1 rounded-xl bg-[var(--os-accent)] hover:bg-[var(--os-accent)]/90 text-[var(--os-accent-fg)] font-bold text-xs py-5"
                >
                ...
                <Button 
                  variant="outline" 
                  onClick={resetForm} 
                  className="rounded-xl border-[var(--os-muted)]/40 text-[var(--os-muted)] hover:text-[var(--os-fg)] bg-transparent text-xs py-5 px-5"
                >
                  Hủy
                </Button>
              </div>
