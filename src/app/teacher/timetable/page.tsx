    ...
    <TeacherShell onLogout={async () => { await supabase.auth.signOut(); router.push("/login") }}>
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10 pt-20 lg:pt-10 pb-24">
        <section className="mb-8 grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
          ...
          <div className="rounded-2xl border border-[hsl(var(--border))]/60 bg-[hsl(var(--card))] p-6"><p className="text-sm text-[hsl(var(--muted-foreground))]">Tổng tiết</p><div className="mt-2 text-3xl font-semibold">{entries.length}</div><p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">Đang hiển thị</p></div>
        </section>
