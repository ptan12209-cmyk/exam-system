...
export default function ArenaAdminPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch arena sessions and their participant count
      ...
      console.error("Error fetching admin arena data:", err)
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const query = editingId
        ? supabase.from("arena_sessions").update(data).eq("id", editingId)
        : supabase.from("arena_sessions").insert(data)
  ...
  const getStatusBadge = (session: ArenaSession) => {
    ...
    if (now < start) {
      return (
        <span className="inline-flex items-center rounded-full bg-indigo-100 px-2.5 py-0.5 text-xs font-medium text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300">
          <Clock className="mr-1 h-3 w-3" /> Sắp diễn ra
        </span>
      )
    }
    if (now >= start && now <= end) {
      return (
        <span className="inline-flex items-center rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
          <Play className="mr-1 h-3 w-3 animate-pulse" /> Đang diễn ra
        </span>
      )
    }
    return (
      <span className="inline-flex items-center rounded-full bg-[hsl(var(--muted))]/20 px-2.5 py-0.5 text-xs font-medium text-[hsl(var(--muted-foreground))]">
        <CheckCircle className="mr-1 h-3 w-3" /> Đã kết thúc
      </span>
    )
  }
  ...
  const examOptions = useMemo(() => {
    return exams.map((exam) => ({
      value: exam.id,
      label: `${exam.title} (${exam.total_questions} câu - ${exam.subject})`,
    }))
  }, [exams])
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10 pt-20 lg:pt-10 pb-24">
        ...
        <section className="mb-8 grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
          <div>
            ...
            <h1 className="text-4xl font-semibold tracking-tight md:text-6xl">Quản lý Đấu trường</h1>
        ...
        <div className="mb-6">
          ...
          >
            <Plus className="mr-2 h-4 w-4" /> Tạo đợt thi
          </Button>
        </div>
            ...
            {sessions.map((session) => (
              ...
              >
                <div className="border-b border-[hsl(var(--border))]/50 p-5">
                  <div className="mb-2 flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      {session.exam && (
                        <div className="mt-2 inline-flex items-center rounded-lg bg-violet-50 dark:bg-violet-950/40 px-2 py-0.5 text-xs font-medium text-violet-700 dark:text-violet-300">
                          <FileText className="mr-1 h-3 w-3" /> {session.exam.title}
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-4 p-5">
                  ...
                  <div className="flex gap-2 pt-2 border-t border-[hsl(var(--border))]/20">
                    <Button
                      variant="outline"
                      size="sm"
              ...
              <div className="p-5 overflow-y-auto max-h-[80vh]">
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    {!exams.length && (
                      <p className="rounded-xl border border-amber-200 dark:border-amber-950/40 bg-amber-50 dark:bg-amber-950/20 p-2.5 text-xs text-amber-700 dark:text-amber-300">
                        ⚠ Bạn chưa có đề thi nào.{" "}
                        <Link href="/teacher/exams/create" className="underline font-semibold">
                          Tạo ngay
                        </Link>
                      </p>
                    )}
                  ...
                  <div className="flex gap-3 border-t border-[hsl(var(--border))]/50 pt-4 mt-6">
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => {
