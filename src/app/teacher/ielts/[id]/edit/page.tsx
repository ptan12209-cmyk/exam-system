...
export default function EditIeltsTestPage(props: { params: Promise<{ id: string }> }) {
  ...
  if (error || !test) {
    return (
      <div className="container mx-auto p-6 text-center max-w-xl space-y-4">
        ...
        <Link href="/teacher/ielts" className="inline-block px-5 py-2.5 rounded-xl border border-white/10 text-sm font-semibold text-muted-foreground hover:text-foreground">
          Quay lại danh sách
        </Link>
      </div>
    )
  ...
  const handleSaveDetails = async (e: React.FormEvent) => {
    ...
    if (success) {
      alert('Đã cập nhật cài đặt đề thi thành công!')
    }
  }
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 space-y-6 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
        ...
        {/* Tab Controls */}
        <div className="flex gap-1.5 bg-white/5 p-1 rounded-xl border border-white/5 self-start">
          <button
      ...
      <div className="space-y-4">
        {/* Tab Details */}
        {activeTab === 'details' && (
          <form onSubmit={handleSaveDetails} className="glass-card p-6 rounded-2xl border border-white/10 bg-neutral-900/40 space-y-4 max-w-2xl">
            <h3 className="text-sm font-bold text-cyan-400 border-b border-white/10 pb-2 mb-4">Cài đặt cơ bản bài thi</h3>
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-foreground focus:outline-none focus:border-cyan-500/50"
              />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-foreground focus:outline-none cursor-pointer [&>option]:bg-neutral-900"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-foreground focus:outline-none disabled:opacity-50"
                />
              </div>
            </div>
            ...
            <div className="pt-4 flex items-center justify-end">
              <button
                type="submit"
                disabled={saving}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-semibold text-sm flex items-center justify-center gap-2 shadow-lg transition-all"
              >
