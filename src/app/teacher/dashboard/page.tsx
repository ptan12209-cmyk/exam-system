...
import { BarChart3, BookOpen, FileText, Users, Clock, Plus } from "lucide-react"
    ...
    <TeacherShell onLogout={handleLogout}>
      ...
      <main className="mx-auto max-w-7xl px-4 pb-24 pt-24 sm:px-6 lg:px-8 lg:py-10">
        ...
        <section className="mt-10 overflow-hidden rounded-[2rem] liquid-glass shadow-[0_30px_80px_-40px_rgba(0,0,0,0.35)]">
          ...
          {filteredExams.length === 0 ? <div className="py-16 text-center text-[hsl(var(--muted-foreground))]">Chưa có đề thi phù hợp</div> : <div className="divide-y divide-[hsl(var(--border))]/40">{filteredExams.map((exam) => { const subjectInfo = getSubjectInfo(exam.subject || "other"); return (<div key={exam.id} className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-start gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[hsl(var(--border))]/60">{subjectInfo.icon}</div><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-base font-semibold">{exam.title}</h3><span className={cn("rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.2em]", exam.status === "published" ? "border border-emerald-200 bg-emerald-50 text-emerald-700" : "border border-amber-200 bg-amber-50 text-amber-700")}>{exam.status === "published" ? "Đã phát hành" : "Nháp"}</span></div><div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-[hsl(var(--muted-foreground))]"><span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" />{exam.duration} phút</span><span className="flex items-center gap-1"><FileText className="h-3.5 w-3.5" />{exam.total_questions} câu</span><span className="flex items-center gap-1"><Users className="h-3.5 w-3.5" />{exam.submission_count || 0} lượt nộp</span></div></div></div><div className="flex items-center gap-2 self-start lg:self-auto"><Link href={`/teacher/exams/${exam.id}/scores`}><Button variant="outline" size="sm" className="rounded-full border-[hsl(var(--border))]/70 bg-transparent">Kết quả</Button></Link><Link href={`/teacher/exams/${exam.id}/edit`}><Button variant="outline" size="sm" className="rounded-full border-[hsl(var(--border))]/70 bg-transparent">Sửa</Button></Link></div></div>)})}</div>}
        </section>
      </main>
