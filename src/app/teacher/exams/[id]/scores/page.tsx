    ...
    <TeacherShell onLogout={async () => { await supabase.auth.signOut(); router.push("/login") }}>
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10 pt-24 lg:pt-10 pb-24">
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 mb-6">
          ...
          ].map(({ icon: Icon, value, label }) => <div key={label} className="rounded-[2rem] border border-[hsl(var(--border))]/60 bg-[hsl(var(--card))] p-4"><div className="flex items-center gap-3"><div className="rounded-2xl bg-[hsl(var(--muted))]/20 p-2"><Icon className="h-5 w-5" /></div><div><p className="text-2xl font-semibold">{value}</p><p className="text-sm text-[hsl(var(--muted-foreground))]">{label}</p></div></div></div>)}
        </section>
        ...
        <section className="rounded-[2rem] border border-[hsl(var(--border))]/60 bg-[hsl(var(--card))] overflow-hidden">
          ...
          {submissions.length === 0 ? <div className="py-16 text-center text-[hsl(var(--muted-foreground))]"><Users className="mx-auto mb-4 h-12 w-12 opacity-30" />Chưa có học sinh nào nộp bài</div> : <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-[hsl(var(--border))]/50 bg-[hsl(var(--muted))]/10 text-left">{["Hạng", "Học sinh", "Điểm", "Số câu đúng", "Thời gian", "Nộp lúc", "Chi tiết"].map((h) => <th key={h} className="p-4 text-center text-xs font-semibold uppercase text-[hsl(var(--muted-foreground))]">{h}</th>)}</tr></thead><tbody className="divide-y divide-[hsl(var(--border))]/30">{submissions.map((sub, index) => <tr key={sub.id} className="transition-colors hover:bg-[hsl(var(--muted))]/20"><td className="p-4 text-center"><span className="mx-auto inline-flex h-7 w-7 items-center justify-center rounded-full border text-xs font-semibold">{index + 1}</span></td><td className="p-4"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[hsl(var(--foreground))] text-[hsl(var(--background))] font-semibold">{sub.profile?.full_name?.charAt(0) || sub.student_id.charAt(0)}</div><div><p className="font-medium">{sub.profile?.full_name || `Học sinh ${sub.student_id.slice(0, 8)}`}{violations[sub.student_id] && <span title={`Vi phạm: ${violations[sub.student_id].total}`} className="ml-2 inline-flex items-center gap-1 rounded-full border border-red-200 bg-red-50 px-1.5 py-0.5 text-[10px] font-semibold text-red-600"><ShieldAlert className="h-3 w-3" />{violations[sub.student_id].total}</span>}</p><p className="text-xs text-[hsl(var(--muted-foreground))] font-mono">#{sub.student_id.slice(0, 8)}</p></div></div></td><td className="p-4 text-center"><span className={cn("inline-block min-w-[3rem] rounded-full border px-3 py-1 text-sm font-bold", getScoreColor(sub.score))}>{sub.score.toFixed(1)}</span></td><td className="p-4 text-center text-sm"><span className="text-emerald-600">{sub.correct_count}</span>/<span className="text-[hsl(var(--muted-foreground))]">{exam.total_questions}</span></td><td className="p-4 text-center"><span className="inline-flex items-center gap-1 rounded-xl border border-[hsl(var(--border))]/50 bg-[hsl(var(--muted))]/20 px-2.5 py-1 text-xs text-[hsl(var(--muted-foreground))]"><Clock className="h-3.5 w-3.5" />{formatTime(sub.time_spent)}</span></td><td className="p-4 text-right text-sm text-[hsl(var(--muted-foreground))]">{formatDate(sub.submitted_at)}</td><td className="p-4 text-center"><Link href={`/teacher/exams/${examId}/submissions/${sub.id}`}><Button variant="ghost" size="icon" className="rounded-full"><Eye className="h-5 w-5" /></Button></Link></td></tr>)}</tbody></table></div>}
        </section>
      </main>
      <TeacherBottomNav />
    </TeacherShell>
